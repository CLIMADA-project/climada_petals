...
from climada_petals.engine.supplychain.mriot_handling import MRIOT_MONETARY_FACTOR, MRIOT_YEAR_REGEX, WIOD_FILE_LINK, build_eora_from_zip, build_exio3_from_zip, build_oecd_from_csv, download_mriot, lexico_reindex, parse_mriot, parse_mriot_from_df, parse_wiod_v2016
...
def test_lexico_reindex(example_mriot):
    ...
    pd.testing.assert_frame_equal(sorted_mriot.Z, example_mriot.Z.sort_index(axis=0).sort_index(axis=1))
    pd.testing.assert_frame_equal(sorted_mriot.Y, example_mriot.Y.sort_index(axis=0).sort_index(axis=1))
    pd.testing.assert_frame_equal(sorted_mriot.x, example_mriot.x.sort_index(axis=0).sort_index(axis=1))
    pd.testing.assert_frame_equal(sorted_mriot.A, example_mriot.A.sort_index(axis=0).sort_index(axis=1))
...
class TestBuildExio3FromZip(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_io_system.get_regions = MagicMock(
            return_value=pd.Index(["Region A", "Region B", 'WA', 'WE', 'WF', 'WL', 'WM', "ZA"])
        )
        self.mock_io_system.aggregate = MagicMock(return_value=self.mock_io_system)

        # Mocking return values for parse_exiobase3
        patcher_parse = patch('pymrio.parse_exiobase3', return_value=self.mock_io_system)
        self.addCleanup(patcher_parse.stop)
        self.mock_parse_exiobase3 = patcher_parse.start()

        patcher_reindex = patch('climada_petals.engine.supplychain.mriot_handling.lexico_reindex', return_value=self.mock_io_system)
    ...
    def test_build_exio3_from_zip(self):
        ...
        mrio = build_exio3_from_zip(self.mock_zip_path, remove_attributes=False, aggregate_ROW=True)

        # Ensure parse_exiobase3 was called correctly
        self.mock_parse_exiobase3.assert_called_once_with(path=pathlib.Path(self.mock_zip_path))
        ...
        self.mock_io_system.meta.change_meta.assert_any_call("name", "EXIOBASE3")
        self.mock_io_system.meta.change_meta.assert_any_call("monetary_factor", MRIOT_MONETARY_FACTOR["EXIOBASE3"])
        self.mock_io_system.meta.change_meta.assert_any_call("year", self.mock_io_system.meta.description[-4:])
        self.mock_io_system.meta.change_meta.assert_any_call("basename", "exiobase3_ixi")
        self.mock_io_system.meta.change_meta.assert_any_call("sectors_agg", "full_sectors")
        self.mock_io_system.meta.change_meta.assert_any_call("regions_agg", "full_regions")

    def test_build_exio3_without_removing_attributes(self):
        # Call the function with remove_attributes=False
        build_exio3_from_zip(self.mock_zip_path, remove_attributes=False, aggregate_ROW=False)
...
class TestBuildEoraFromZip(unittest.TestCase):
    ...
    def test_build_eora_from_zip_default(self):
        ...
        self.mock_parse_eora26.assert_called_once_with(path=pathlib.Path(self.mock_zip_path))
        ...
        self.assertEqual(mrio.year, re.search(MRIOT_YEAR_REGEX, self.mock_zip_path)["mrio_year"])
        self.assertEqual(mrio.sectors_agg, "full_sectors")
        self.assertEqual(mrio.regions_agg, "full_regions")

        # Ensure the meta attributes are updated
        self.mock_io_system.meta.change_meta.assert_any_call("monetary_factor", MRIOT_MONETARY_FACTOR["EORA26"])
        self.mock_io_system.meta.change_meta.assert_any_call("year", self.mrio_year_match["mrio_year"])
    ...
    def test_build_eora_with_reexport_treatment(self):
        ...
        mrio = build_eora_from_zip(self.mock_zip_path, reexport_treatment=True, remove_attributes=False)

        # Ensure the re-export treatment is applied
        self.mock_io_system.rename_sectors.assert_called_once_with({"Re-export & Re-import": "Others"})
    ...
    def test_build_eora_with_inventory_treatment(self):
        ...
        mriot = build_eora_from_zip(self.mock_zip_path, inv_treatment=True, remove_attributes=False)
    ...
    def test_build_eora_without_inventory_treatment(self):
        ...
        build_eora_from_zip(self.mock_zip_path, inv_treatment=False, remove_attributes=False)
...
class TestBuildOECDFromCSV(unittest.TestCase):
    ...
    def test_build_oecd_from_csv(self):
        ...
        mrio = build_oecd_from_csv(self.mock_csv_path, year=2020, remove_attributes=False)

        # Ensure parse_oecd was called correctly
        self.mock_parse_oecd.assert_called_once_with(path=pathlib.Path(self.mock_csv_path), year=2020)
        ...
        self.assertEqual(mrio.year, re.search(MRIOT_YEAR_REGEX, self.mock_csv_path)["mrio_year"])
        self.assertEqual(mrio.sectors_agg, "full_sectors")
        self.assertEqual(mrio.regions_agg, "full_regions")

        # Ensure the meta attributes are updated
        self.mock_io_system.meta.change_meta.assert_any_call("monetary_factor", MRIOT_MONETARY_FACTOR["OECD23"])
        self.mock_io_system.meta.change_meta.assert_any_call("year", re.search(MRIOT_YEAR_REGEX, self.mock_csv_path)["mrio_year"])
...
class TestParseWIODv2016(unittest.TestCase):
    ...
    def test_parse_wiod_v2016(self):
        ...
        self.mock_read_excel.assert_called_once_with(self.mock_xlsb_path, engine="pyxlsb")
        ...
        expected_multiindex = pd.MultiIndex.from_product(
            [self.mock_io_system.get_regions.return_value, self.mock_io_system.get_sectors.return_value],
            names=["region", "sector"]
        )
        pd.testing.assert_frame_equal(
        ...
        self.assertEqual(mrio.year, re.search(MRIOT_YEAR_REGEX, self.mock_xlsb_path)["mrio_year"])
        self.assertEqual(mrio.sectors_agg, "full_sectors")
        self.assertEqual(mrio.regions_agg, "full_regions")

        # Ensure the meta attributes are updated
        self.mock_io_system.meta.change_meta.assert_any_call("monetary_factor", MRIOT_MONETARY_FACTOR["WIOD16"])
        self.mock_io_system.meta.change_meta.assert_any_call("year", re.search(MRIOT_YEAR_REGEX, self.mock_xlsb_path)["mrio_year"])
...
class TestParseFromDF(unittest.TestCase):
        ...
        expected_Y = pd.DataFrame.from_dict(
            {
                ...
                "columns": [("USA", "FD_1"), ("USA", "FD_2"), ("CHN", "FD_1"), ("CHN", "FD_2")],
                "data": [
    ...
    def test_parse_mriot_from_df_no_fd_cat(self):
        mriot_df, col_iso3, col_sectors, rows_data, cols_data, _ = (
            self.dummy_mriot_df()
        )
        Z, Y, x = parse_mriot_from_df(
        ...
        expected_Y = pd.DataFrame.from_dict(
            {
                ...
                "columns": [("USA", "fd_cat_0"), ("USA", "fd_cat_1"), ("CHN", "fd_cat_0"), ("CHN", "fd_cat_1")],
                "data": [
                    [300.0, 100.0, 300.0, 100.0],
                    [400.0, 200.0, 400.0, 200.0],
                    [500.0, 300.0, 500.0, 300.0],
...
class TestDownloadMRIOT(unittest.TestCase):
    ...
    def test_download_oecd21(self, mock_download_oecd):
        # Mock parameters
        mriot_type = "OECD21"
        mriot_year = 2007
        download_dir = pathlib.Path("/fake/dir")
        ...
        mock_download_oecd.assert_called_once_with(
            storage_folder=download_dir, years="2005-2009"
        )

class TestParseMriot(unittest.TestCase):
    @patch('pymrio.parse_exiobase3')
    def test_parse_mriot_exiobase3(self, mock_parse_exiobase3):
        # Mock return object for pymrio.parse_exiobase3
        mock_mriot = MagicMock()
        mock_parse_exiobase3.return_value = mock_mriot

        downloaded_file = pathlib.Path("dummy_path")
        mriot = parse_mriot("EXIOBASE3", downloaded_file, 2010)

        # Assert pymrio.parse_exiobase3 was called correctly
        mock_parse_exiobase3.assert_called_once_with(path=downloaded_file)

        # Assert mriot.A was set to None
        self.assertIsNone(mock_mriot.A)

    @patch('pandas.read_excel')
    @patch('pymrio.IOSystem')
    @patch('climada_petals.engine.supplychain.utils.parse_mriot_from_df')
    def test_parse_mriot_wiod16(self, mock_parse_mriot_from_df, mock_IOSystem, mock_read_excel):
        # Mock objects
        mock_mriot_df = MagicMock()
        mock_read_excel.return_value = mock_mriot_df
        mock_Z, mock_Y, mock_x = MagicMock(), MagicMock(), MagicMock()
        mock_parse_mriot_from_df.return_value = (mock_Z, mock_Y, mock_x)
        mock_io_system_instance = MagicMock()
        mock_IOSystem.return_value = mock_io_system_instance

        downloaded_file = pathlib.Path("dummy_path")
        mriot = parse_mriot("WIOD16", downloaded_file, 2010)

        # Assert pandas.read_excel was called correctly
        mock_read_excel.assert_called_once_with(downloaded_file, engine="pyxlsb")

        # Assert parse_mriot_from_df was called with correct arguments
        mock_parse_mriot_from_df.assert_called_once_with(
            mock_mriot_df, col_iso3=2, col_sectors=1, row_fd_cats=2, rows_data=(5, 2469), cols_data=(4, 2468)
        )

        # Assert pymrio.IOSystem was initialized with Z, Y, and x
        mock_IOSystem.assert_called_once_with(Z=mock_Z, Y=mock_Y, x=mock_x)

        # Assert change_meta method was called on the mriot object
        mock_io_system_instance.meta.change_meta.assert_called_once_with("name", "WIOD16")

    @patch('pymrio.parse_oecd')
    @patch('pymrio.calc_x')
    def test_parse_mriot_oecd21(self, mock_calc_x, mock_parse_oecd):
        # Mock return object for pymrio.parse_oecd
        mock_mriot = MagicMock()
        mock_parse_oecd.return_value = mock_mriot

        downloaded_file = pathlib.Path("dummy_path")
        mriot = parse_mriot("OECD21", downloaded_file, 2010)

        # Assert pymrio.parse_oecd was called correctly
        mock_parse_oecd.assert_called_once_with(path=downloaded_file)

        # Assert pymrio.calc_x was called correctly to compute mriot.x
        mock_calc_x.assert_called_once_with(mock_mriot.Z, mock_mriot.Y)
        self.assertEqual(mriot.x, mock_calc_x.return_value)
