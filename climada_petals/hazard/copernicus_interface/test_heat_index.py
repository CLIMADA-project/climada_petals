...
class TestSeasonalForecastCalculations(unittest.TestCase):
    ...
    def test_calculate_relative_humidity_percent(self):
        ...
        expected = np.array([50.7, 50.3])  # Approximate values
        result = calculate_relative_humidity_percent(t2k, tdk)
        npt.assert_almost_equal(result, expected, decimal=1)
    ...
    def test_calculate_heat_index_simplified(self):
        ...
        self.assertEqual(result.shape, t2k.shape)  # Check shape consistency
    ...
    def test_calculate_relative_humidity(self):
        ...
        # Test inputs
        t2_k = np.array([300.15, 303.15])  # Kelvin (27°C, 30°C)
        td_k = np.array([295.15, 298.15])  # Kelvin (22°C, 25°C)

        # Expected output (manually calculated or verified with trusted method)
        expected_rh = np.array([69.96, 68.29])  # Approximate relative humidity in %
    ...
    def test_calculate_tr(self):
        """Test Tropical Nights (TR) calculation."""
        ...
        expected = xr.DataArray([False, True, False, True], dims=["time"])
        xr.testing.assert_equal(result, expected)

    def test_calculate_tx30(self):
        """Test TX30 calculation."""
        temperature_data = xr.DataArray(
            data=[29, 30, 31],
            dims=["time"],
        ...
        expected = xr.DataArray([False, False, True], dims=["time"])
        xr.testing.assert_equal(result, expected)
    ...
    def test_calculate_hw(self):
        ...
        expected = np.array([0, 1, 1, 0, 1, 1, 1])  # Indicating heatwave days
        npt.assert_array_equal(result, expected)
